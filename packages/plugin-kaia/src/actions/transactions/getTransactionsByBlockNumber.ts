...
export const getTransactionsByBlockNumberAction: Action = {
    ...
    handler: async (
        ...
        const hasBlockNumber =
            content?.blocknumber && !content?.error;
        ...
        try {
            const kaiaScanData = await kaiaScanService.getTransactionsByBlockNumber(String(content.blocknumber));
            elizaLogger.success(
            ...
            if (callback) {
                ...
                if (kaiaScanData && kaiaScanData.results.length > 0) {
                    BlockTransactions = kaiaScanData.results.map((transaction) => {
                        return {
